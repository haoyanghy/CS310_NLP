...
def load_glove_embeddings(vocab, embedding_dim=200):
    glove_path = (
        "/home/tangm_lab/cse12212027/NLP/gensim-data/glove-wiki-gigaword-200.gz"
    )
...
if __name__ == "__main__":
    ...
    lstm_glove = LSTMLanguageModel(
        vocab_size, embedding_dim, hidden_dim, num_layers, dropout, glove_embeddings
    )
    print("\nTraining LSTM with GloVe embeddings...")
    lstm_glove, glove_losses = train_model(
        lstm_glove, train_loader, val_loader, num_epochs, device
    )
    ...
    lstm_model, lstm_losses = train_model(
        lstm_model, train_loader, val_loader, num_epochs, device
    )
    ...
    for prefix in prefixes:
        lstm_sentence = generate_sentence(
            lstm_model, prefix, vocab, idx_to_word, device=device
        )
